...
export default function Form({
  ...
  const pickImage = async () => {
    ...
    if (!result.cancelled) {
      setForm({ ...form, image_source: result.uri, imageChange: true });
    }
  };
  ...
  const pickDocument = async () => {
    ...
    if (result.type == "success") {
      setForm({ ...form, audio_source: result, audioChange: true });
    }
  };
    ...
    <KeyboardAvoidingView behavior="height" style={styles.container}>
      <ScrollView>
        ...
        <Button status="primary" size="medium" onPress={pickDocument}>
          {form.audio_source && form.audio_source.name ? (
            <Text category="s1">{form.audio_source.name}</Text>
          ) : (
